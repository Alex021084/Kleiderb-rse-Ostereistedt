...
function normalize(s){return {...s,commissionEnabled:s.commissionEnabled===undefined?s.commission_enabled!==false:s.commissionEnabled,commissionRate:Number(s.commissionRate??s.commission_rate??15),phone:s.phone||""}}
...
async function saveSeller(){
  let number=$("number").value.trim(),name=$("name").value.trim(),phone=$("phone").value.trim(),commissionEnabled=commissionInput?commissionInput.checked:true,commissionRate=commissionRateValue();
  ...
  Object.assign(s,{number,name,phone,commissionEnabled,commissionRate});
  try{
    if(KBCloud.cloudReady()){await KBCloud.cloudSaveSeller(s)}else{let i=sellers.findIndex(x=>x.id===s.id);if(i<0)sellers.push(s);localStorage.setItem("kb_sellers",JSON.stringify(sellers))}
