...
function sorted(list){
  return [...list].sort((a,b)=>{
    const ap=a.name.trim().split(/\s+/).pop().toLocaleLowerCase("de-DE");
    const bp=b.name.trim().split(/\s+/).pop().toLocaleLowerCase("de-DE");
...
function render(){
  ...
  const filtered=sorted(sellers).filter(s=>s.name.toLocaleLowerCase("de-DE").includes(query)||s.number.includes(query));
  $("count").textContent=`${sellers.length} ${sellers.length===1?"Verkäufer":"Verkäufer"}`;

  if(!filtered.length){
    $("list").innerHTML=sellers.length
      ? '<div class="empty"><strong>Kein Treffer</strong>Versuche einen anderen Namen oder eine andere Verkäufernummer.</div>'
      : '<div class="empty"><strong>Noch keine Verkäufer</strong>Lege den ersten Verkäufer an.</div>';
    return;
  }
...
function openForm(){
  editId=null;$("formTitle").textContent="Verkäufer anlegen";
  $("number").value="";$("name").value="";$("phone").value="";$("error").textContent="";
...
function saveSeller(){
  ...
  const duplicate=sellers.find(s=>s.number===number&&s.id!==editId);
  if(duplicate){$("error").textContent="Diese Verkäufernummer ist bereits vergeben.";return}
  if(editId){
    const s=sellers.find(x=>x.id===editId);s.number=number;s.name=name;s.phone=phone;
    toast("Verkäufer geändert.");
  }else{
    sellers.push({id:crypto.randomUUID?crypto.randomUUID():String(Date.now()+Math.random()),number,name,phone});
    toast("Verkäufer angelegt.");
  }
  save();closeForm();render();
}
function deleteSeller(id){
  const s=sellers.find(x=>x.id===id);if(!s)return;
  if(confirm(`Verkäufer "${s.name}" wirklich löschen?`)){sellers=sellers.filter(x=>x.id!==id);save();render();toast("Verkäufer gelöscht.")}
}
function goHome(){
  // Für den Test geht es zurück zur Startseite des gleichen GitHub-Ordners.
